...
class LLMProcessor:
    ...
    def _build_primary_url(self) -> str:
    ...
    def _maybe_build_fallback_urls(self) -> list:
        """
        仅用于兼容少量“base_url 没带 /v1”的历史配置场景。
        默认情况下不做过度猜测，避免把错误信息变得更混乱。
        """
        base = (self.base_url or "").rstrip("/")
        urls: list = []
        if not base:
    ...
    def generate(self, prompt: str) -> str:
        ...
        fallback_urls = self._maybe_build_fallback_urls()
        ...
        for attempt in range(self.retry):
            try:
                ...
                if self._is_auth_error(response) and not enable_fallback:
                    raise Exception(self._format_auth_error(response))

                if self._is_auth_error(response) and enable_fallback:
                    first_auth_error = self._format_auth_error(response)
                    for u in fallback_urls:
                        for hdr in fallback_headers:
                            r2 = requests.post(u, headers=hdr, json=payload, timeout=self.timeout)
                            if 200 <= int(r2.status_code or 0) < 300:
                                result_json = r2.json()
                                return result_json["choices"][0]["message"]["content"]
                    raise Exception(f"{first_auth_error}（已尝试备用鉴权头/URL 仍失败）")
