...
/**
 * 表格分析（顶层页面）：允许用户批量勾选历史记录，将其合并导入到测试表结构中做编辑/导出。
 */
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setError(e?.response?.data?.detail || e?.message || '加载历史记录失败');
        setEvents([]);
      } finally {
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 transform transition-all duration-500 animate-in fade-in slide-in-from-bottom-4">
        <Card className="lg:col-span-5 h-fit sticky top-28">
          <h2 className="text-2xl font-serif font-bold mb-6">表格分析</h2>

          <div className="space-y-4">
            <Input label="搜索历史记录" type="text" value={query} onChange={(e: any) => setQuery(e.target.value)} placeholder="按标题或 URL 搜索" />
            ...
            {loading ? <div className="text-sm text-gray-500">正在加载历史记录...</div> : null}

            {!loading && events.length === 0 ? <div className="text-sm text-gray-500">暂无历史记录</div> : null}
